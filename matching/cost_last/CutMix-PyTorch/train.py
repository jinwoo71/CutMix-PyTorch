...
# Check
writer = SummaryWriter('/home_goya/jinwoo.choi/runs250/cost_last/')
...
def main():
    ...
    print("args")
    global decoder, vgg, pretrained, mat250c2
    decoder = net.decoder
    vgg = net.vgg
    decoder.eval()
    vgg.eval()
    ...
    vgg.cuda()
    decoder.cuda()
    mat250c2 = torch.load('mat250c2_last.pt', map_location='cuda:0')
    mat250c2 = mat250c2[2].cpu()
    ind = torch.arange(mat250c2.shape[1])
    mat250c2[ind, ind] += 1
    global network
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        if r < 0.5:
            ...
            if args.method == 'content_style_mixup_loss_labeling' :
                target_temp = mat250c2[:,target.view(-1)]
                target_mat = target_temp[target.view(-1),:]
                row_ind, col_ind = linear_sum_assignment(target_mat)
                target_a_new = target[row_ind]
                target_b_new = target[col_ind]
                content = input[row_ind]
                style = input[col_ind]
                x = np.random.uniform()
                y = np.random.uniform()
                with torch.no_grad():
                ...
                a_loss = -log_preds[torch.arange(output.shape[0]),target_a_new] # cross-entropy for A
                b_loss = -log_preds[torch.arange(output.shape[0]),target_b_new] # cross-entropy for B
