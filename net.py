...
class Net_D(nn.Module):
    ...
    """
    def forward(self, content, style, content_feat, style_feat, alpha, beta):
        assert 0 <= alpha <= 1
        x = alpha
        y = beta
        t = np.random.uniform(max(0, x+y-1), min(x, y), 1)[0]

        g_t = self.decoder(t * content_feat + (1.0-x-y+t) * style_feat + (x-t) * adain(content_feat, style_feat) + (y-t) * adain(style_feat, content_feat))
        return g_t
    """
    def forward(self, content, style, content_feat, style_feat, alpha, beta, gamma, bbx1, bby1, bbx2, bby2):
        assert 0 <= alpha <= 1
        x = alpha
        y = beta
        z = gamma
